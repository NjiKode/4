...
export async function before(m, {isOwner}) {
    ...
    let isSurrend = (/(me)?nyerah|surrend(e(r|d))?/i.test(m.text) && (m.sender === this.susunkata[id][4] || isOwner))
    if (m.quoted.id == this.susunkata[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
          ...
          await this.sendButton(m.chat, `Yaahh Menyerah :(\nJawaban yg benar adalah *${json.jawaban}*`, author, null, [['Susun Kata', '.susunkata']], m)
          clearTimeout(this.susunkata[id][3])
          delete this.susunkata[id]
